...
export function Button(props: ButtonProps) {
  const { type = "transparent", onClick, children, style } = props
  if (type === "arbitrary") {
    return (
      <button className="flex items-center justify-center min-w-[160px] py-2 rounded-full" style={style} onClick={onClick}>
        {children}
      </button>
    )
  } else if (type === "close") {
    return (
      <button className="flex items-center justify-center p-2 rounded-full" style={colorSwitch(type)} onClick={onClick}>
    ...
    return (
      <button className="text-sm text-blue-600" style={style} onClick={onClick}>
        {children}
      </button>
    )
  }  else if (type === "submit") {
    return (
      <button className="flex items-center justify-center py-2 px-4 rounded-full bg-blue-600 text-white" style={style} onClick={onClick} type="submit">
        {children}
      </button>
    )
  } else {
    return (
      <button className="flex items-center justify-center min-w-[160px] py-2 rounded-full" style={colorSwitch(type)} onClick={onClick}>
        {textSwitch(type)}
      </button>
    )
  }
